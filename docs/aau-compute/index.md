...
AAU Compute is a GPU cluster made up of a collecton of NVIDIA GPU's, designed for processing GPU-demanding machine learning workloads. The platform is accessed through a terminal application on the user's local machine. From here the user logs in to a front end node, where files management and job submission to the compute nodes takes place.

!!! info "Not for confidential data"
    With AAU Compute you are only allowed to work with data that is **not confidential** according to [AAU’s data classification model](https://www.security.aau.dk/data-classification){target="_blank} (classified as level 1).
...
[Get access to AAU Compute](/aau-compute/how-to-access){ .md-button .md-button--primary }
