# Mistral

<div class="tag-container">
    <div class="tag-item">
        <div class="tag">Researchers<i class="bi bi-check-lg"></i></div>
        <div class="tooltip">Indicates if the service is relevant for researchers working with writing, ideation, analysis, or AI-assisted workflows.</div>
    </div>
    <div class="tag-item">
        <div class="tag">Students<i class="bi bi-x-lg"></i></div>
        <div class="tooltip">Student availability should be confirmed before publishing this page broadly.</div>
    </div>
    <div class="tag-item">
        <div class="tag">Sensitive Data<i class="bi bi-x-lg"></i></div>
        <div class="tooltip">Do not assume the service is approved for confidential or sensitive data unless this has been explicitly confirmed.</div>
    </div>
    <div class="tag-item">
        <div class="tag">Web interface<i class="bi bi-check-lg"></i></div>
        <div class="tooltip">The service is accessed through a browser-based interface.</div>
    </div>
    <div class="tag-item">
        <div class="tag">Pre-installed apps<i class="bi bi-x-lg"></i></div>
        <div class="tooltip">Mistral is a software service, not a platform with installable applications for end users.</div>
    </div>
    <div class="tag-item">
        <div class="tag">Collaboration friendly<i class="bi bi-x-lg"></i></div>
        <div class="tooltip">Collaboration features should be described only once the AAU-supported workflow is known.</div>
    </div>
 </div>

## Introduction

Mistral is an AI service that gives AAU researchers access to generative AI tools. In this section you can read more about the two services: **Le Chat** for conversational AI assistance and **AI Studio** for larger generative tasks, including API-based integration with custom AI workflows.

## Getting Started

<div class="custom-grid-3">
    <a href="/mistral/how-to-access/" class="custom-grid-item">
        <i class="custom-grid-icon bi bi-file-lock2-fill"></i>
        <p class="custom-grid-title">How to access</p>
        <p class="custom-grid-content">Learn how access to Mistral is expected to work at AAU</p>
    </a>
    <a href="/mistral/guides/le-chat/" class="custom-grid-item">
        <i class="custom-grid-icon bi bi-chat-dots-fill"></i>
        <p class="custom-grid-title">Le Chat</p>
        <p class="custom-grid-content">Start with conversational AI for drafting, ideation, and exploration</p>
    </a>
    <a href="/mistral/guides/ai-studio/" class="custom-grid-item">
        <i class="custom-grid-icon bi bi-sliders2"></i>
        <p class="custom-grid-title">AI Studio</p>
        <p class="custom-grid-content">Work more systematically with prompts, models, and reusable setups</p>
    </a>
    <a href="/mistral/terms-and-conditions/" class="custom-grid-item">
        <i class="custom-grid-icon bi bi-file-text-fill"></i>
        <p class="custom-grid-title">Terms and Conditions</p>
        <p class="custom-grid-content">Get an overview of the Terms and Conditions for using Mistral at AAU</p>
    </a>
</div>

## Key Features

<div class="custom-grid-3">
    <div class="custom-grid-item">
        <h2>Two complementary products</h2>
        <p class="custom-grid-feature">Le Chat is useful for quick, interactive assistance, while AI Studio is better suited for structured prompt development and repeated experimentation.</p>
    </div>
    <div class="custom-grid-item">
        <h2>Browser-based workflow</h2>
        <p class="custom-grid-feature">Users can work directly in the browser without setting up local software, making it easy to test ideas and integrate AI into everyday tasks.</p>
    </div>
    <div class="custom-grid-item">
        <h2>Academic productivity support</h2>
        <p class="custom-grid-feature">Mistral can support drafting, summarization, exploration of concepts, prompt refinement, and prototyping for research workflows.</p>
    </div>
</div>

## Common Use Cases

<div class="custom-grid-3">
    <div class="custom-usecase-item">
        <i class="custom-usecase-icon bi bi-pencil-square"></i>
        <p class="custom-usecase-text">Drafting and rewriting text in Le Chat</p>
    </div>
    <div class="custom-usecase-item">
        <i class="custom-usecase-icon bi bi-journal-text"></i>
        <p class="custom-usecase-text">Summarizing articles, meetings, and project material</p>
    </div>
    <div class="custom-usecase-item">
        <i class="custom-usecase-icon bi bi-sliders"></i>
        <p class="custom-usecase-text">Testing prompts and workflows in AI Studio</p>
    </div>
    <div class="custom-usecase-item">
        <i class="custom-usecase-icon bi bi-plug"></i>
        <p class="custom-usecase-text">Building API-based AI integrations and assistants</p>
    </div>
    <div class="custom-usecase-item">
        <i class="custom-usecase-icon bi bi-file-earmark-text"></i>
        <p class="custom-usecase-text">Document AI and OCR for extracting text and tables from files</p>
    </div>
    <div class="custom-usecase-item">
        <i class="custom-usecase-icon bi bi-collection"></i>
        <p class="custom-usecase-text">Batch processing for summarization, extraction, or classification</p>
    </div>
    <div class="custom-usecase-item">
        <i class="custom-usecase-icon bi bi-braces"></i>
        <p class="custom-usecase-text">Building and testing AI prototypes through the API</p>
    </div>
    <div class="custom-usecase-item">
        <i class="custom-usecase-icon bi bi-easel"></i>
        <p class="custom-usecase-text">Preparing research notes, project outlines, and working drafts</p>
    </div>
    <div class="custom-usecase-item">
        <i class="custom-usecase-icon bi bi-lightbulb"></i>
        <p class="custom-usecase-text">Brainstorming ideas and exploring use cases before implementation</p>
    </div>
</div>

## Important Information

!!! warning "Level 1 data only"
    Only data classified as Level 1 under [AAU's data classification policy](https://www.security.aau.dk/data-classification) may be used with Mistral. Data classified as Level 2 or Level 3 must never be entered into Le Chat or AI Studio. See the full [Terms and Conditions](/mistral/terms-and-conditions/) for details.

!!! info "Choose the right product for the task"
    Use [Le Chat](/mistral/guides/le-chat/) when you need quick conversational assistance. Use [AI Studio](/mistral/guides/ai-studio/) when you want to test prompts more systematically or work with repeatable AI configurations.

!!! info "AI output always requires human review"
    Mistral can save time, but the user remains responsible for checking correctness, bias, citations, compliance, and academic suitability before reusing generated content.
